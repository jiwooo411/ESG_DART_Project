...
[Preprocessing 선택과 그 fragility]                ← 우리 팀 차별점 layer 1
  02_preprocess_kiwi_v2     Kiwi exp_F 토큰화 (canonical choice)
      └── Robustness insets
           · jiwoo/01_tokenizer_robustness_kiwi_vs_okt   (tokenizer instability)
           · jiwoo/02_preprocessing_eval                 (exp_B vs E vs F)
           · jiwoo/01_token_inspection                   (top-N 토큰 직접 읽기)
...
[Feature 구성과 measurement validity]              ← 우리 팀 차별점 layer 2
  ...
  03b_expanded_dictionary_fasttext  fastText 확장 · θ 비교 · 후보 검토
                                    (전처리/ 팀원 notebook 통합)
      └── Robustness inset
           · jiwoo/02_seed_dictionary_validation         (seed별 개별 ρ)
...
[회귀 추정과 instability]                          ← 우리 팀 차별점 layer 3
  05_regression_v2          OLS (M1~M3) + Ordered Logit (M4) + Binary Logit (M5) + VIF
      └── Robustness inset
           · jiwoo/05_sample_definition_sensitivity      (N-curve sign reversal)
...
| 텍스트 처리 | 형태소 분석기 / 불용어 / 사용자 사전 선택 정당화 | `02_preprocess_kiwi_v2` (Kiwi + 사용자 사전 + exp_F 설계) + jiwoo/02 Decision Box |
...
| 측정 validity | Spearman 순위상관, t-test/MWU 검증 | `04_feature_validation_v2` (ρ + Bootstrap CI + MWU) + jiwoo/02_seed_dictionary_validation |
...
| Alpha | 팀이 자체 설계한 추가 검증 | `06_alpha_analysis_v2` (4개 alpha) + jiwoo/05_sample_definition_sensitivity |
...
1. **Preprocessing sensitivity** — `figure2_preprocessing_sensitivity.png`, exp_B/E/F 비교 (jiwoo/02_preprocessing_eval)
2. **Tokenizer instability** — Kiwi vs Okt 8지표 비교 (jiwoo/01_tokenizer_robustness)
3. **Verbosity dominance** — `figure3_verbosity_dominance.png`, Alpha 1 verbosity-adjusted (`06_alpha_analysis_v2`)
4. **Sign reversal** — `alpha3_sign_reversal_exp_F.png`, N-curve zone 분석 (jiwoo/05_sample_definition_sensitivity, Alpha 3)
...
주요 Decision Box 위치:
- 형태소 분석기 선택 (Kiwi vs Mecab vs Okt) — `02_preprocess_kiwi_v2`, jiwoo/02_preprocess_exp_F
