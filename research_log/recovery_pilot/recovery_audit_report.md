# Recovery Audit Report — ESG DART (jiwoo)

**Universe target**: Hyesung company_master.csv — 127 firms × fy{2022, 2023, 2024} = **381 firm-years**
...
| dimension | jiwoo (current) | Hyesung (company_master) |
|---|---|---|
...
Out of user's 81 firms, only **34 firms × 2 fy = 68 firm-years** fall inside Hyesung universe.
Out of user's 243 collected firm-years, **175 firm-years** are *outside* Hyesung universe (extension sample).

## Decision Box — Universe choice
**Alternative**: (A) Adopt Hyesung 381 universe / (B) Keep user 243 / (C) Union both as separate views
**Choice**: (C) — preserve user's 243 native pipeline as `user_native_extension` alongside Hyesung's 381 target.
...
**Limitation**: cross-universe comparisons must be done with care — features were not regenerated under the Hyesung timing rule for OOU rows.

---

## 3. Failure Taxonomy (within Hyesung 381)
...
|---|---|---|
| jiwoo_native (success) | 60 | none — already analysis-ready |
| jiwoo_native_partial | 8 | manual XML re-parse + custom passage filter for holding cos |
...
| jiwoo_native_extension (OOU) | 175 | none — kept as user-native robustness extension |
...
- Provenance: `source_pipeline ∈ {jiwoo_native, jiwoo_native_partial, jiwoo_native_extension, awaiting_refetch_*}`.
