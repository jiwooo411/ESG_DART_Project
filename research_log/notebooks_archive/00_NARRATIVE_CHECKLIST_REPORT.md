...
- **증거 위치:**
  ...
  - `jiwoo/02_preprocessing_eval`, `jiwoo/01_tokenizer_robustness_kiwi_vs_okt`로 cross-reference
...
- **증거 위치:**
  ...
  - `jiwoo/05_sample_definition_sensitivity` — N-curve 본격 분석
...
- **상태:** ✅ 충족
- **증거 위치:**
  - **Tokenizer**: `jiwoo/01_tokenizer_robustness_kiwi_vs_okt` → `final/02` Robustness Inset이 cross-reference
...
- [x] **tokenizer instability** — `jiwoo/01_tokenizer_robustness`, `final/02` cross-ref
...
- [x] **token survival instability** — `jiwoo/01_token_inspection` + `final/02` cross-ref
